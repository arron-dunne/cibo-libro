...
export function IngredientText({
  ...
  function ingredientUsedInStep(ingredient: string, stepText: string): boolean {
    ...
    if (keyword === "mushrooms") console.log({ text, keyword, regex: regex.test(stepText), stepText})
    return regex.test(stepText);
  }

  const active = stepText ? ingredientUsedInStep(text, stepText) : false;

  const baseSize = size === "full" ? "text-[15px] leading-6" : "text-[14px] leading-5";
  ...
  return (
    ...
    <li>
      <label className={`group flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-orange-50 transition ${active ? "bg-orange-200" : "" }`}>
        ...
        {/* text */}
        <span className={`${baseSize} ${textClass} transition-colors duration-300`}>
          {text}
        </span>
