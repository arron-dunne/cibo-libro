...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  const [formState, formAction, isSubmitting] = useActionState(updatePassword, { error: null });
  ...
  return (
    <form action={formAction} className="space-y-4 text-start">
      ...
      >
        {isSubmitting ? <LoaderCircle className="animate-spin" size={24} /> : "Reset Password"}
      </button>
    </form>
  )
}