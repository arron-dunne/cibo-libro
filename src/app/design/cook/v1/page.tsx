// app/cook/page.tsx
// Cibo Libro — Cook Mode (wireframe, static)
// - Mobile-first layout with large readable text
// - Shows intended controls (Prev/Next, Ingredients, Timer, Wake-lock)
// - No interactivity yet: all buttons/indicators are static placeholders
...
export default function Page() {
  ...
  return (
    <div className="min-h-dvh bg-gradient-to-b from-orange-50 to-white text-gray-900">
      {/* Sticky header: back, title, step indicator, wake-lock status */}
      ...
      {/* Main content area */}
      <main className="mx-auto max-w-screen-lg px-4 sm:px-6 pb-[calc(84px+env(safe-area-inset-bottom))] pt-4 sm:pt-6 grid lg:grid-cols-[1fr_320px] gap-6">
        {/* Step card */}
        <section
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-orange-100">
            ...
            {/* Timer pill (static) */}
            <div className="inline-flex items-center gap-2 rounded-full bg-gray-900 text-white px-3 py-1.5 shadow">
        ...
        {/* Ingredients panel (visible on desktop; hinted on mobile via bottom bar) */}
        <aside
      ...
      {/* Mobile Ingredients bar (static “peek” to suggest a sheet) */}
      <div className="lg:hidden fixed inset-x-0 bottom-[84px] z-10 px-4 sm:px-6">
      ...
      >
        <div className="mx-auto max-w-screen-lg px-4 sm:px-6 py-3">
          ...
          {/* Mobile-only wake-lock status (duplicate of header hint) */}
