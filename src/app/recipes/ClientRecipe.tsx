...
type Step = { text: string; timerSec?: number };
type Recipe = {
  id: string;
  title: string;
  description: string;
...
export default function ClientRecipe({ recipe }: { recipe: Recipe }) {
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 md:py-10">
      {/* Playful hero with gradient and soft shapes */}
      <section className="relative overflow-hidden rounded-3xl border border-amber-100 bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50">
        <div className="grid items-stretch gap-0 md:grid-cols-[1.1fr_1fr]">
          ...
          <div className="relative">
            <div className="absolute inset-0 -z-0 bg-[radial-gradient(1200px_600px_at_-10%_-10%,rgba(255,200,150,0.35),transparent)]" />
          ...
          <div className="relative p-5 md:p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              <a
                href="/recipes/edit"
                className="inline-flex items-center rounded-full border border-orange-300 bg-white px-4 py-2 text-sm font-semibold text-orange-700 hover:bg-orange-50"
              >
                Edit recipe
              </a>
              {recipe.sourceUrl ? (
                <a
                  className="inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold text-slate-700 underline-offset-2 hover:underline"
            ...
            {!recipe.isOwned && (
              <p className="mt-2 text-xs text-slate-500">
                Imported recipes remain private or link-only. Only user-authored recipes can be public.
              </p>
            )}
          </div>
        </div>
      ...
      <section className="mt-6 grid gap-6 md:grid-cols-2">
        <Card title="Notes">
          <textarea
            className="min-h-[120px] w-full resize-vertical rounded-2xl border border-amber-200 bg-white px-3 py-2 text-sm outline-none ring-0 focus:border-orange-400"
            placeholder="Add your cooking notes…"
            defaultValue={recipe.notes || ''}
          />
          <div className="mt-3 flex justify-end">
            <button className="inline-flex items-center rounded-full border border-orange-300 bg-white px-4 py-2 text-sm font-semibold text-orange-700 hover:bg-orange-50">
              Save notes
            </button>
          </div>
        </Card>
        ...
        <Card title="Source & Compliance">
          <div className="space-y-3 text-sm">
            ...
            <p className="text-slate-500">
              We respect creators. Imported recipes stay private or link-only. Public pages are for
...
function InfoChip({ label, value, icon }: { label: string; value: string; icon: string }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-800 shadow-[0_1px_0_rgba(0,0,0,0.03)]">
      ...
      {items.map((it, idx) => {
        ...
        return (
          <li key={id} className="flex items-start gap-3">
            <button
              aria-pressed={isOn}
              aria-label={isOn ? 'Uncheck ingredient' : 'Check ingredient'}
              onClick={() => setChecked((m) => ({ ...m, [id]: !m[id] }))}
              className={
                'mt-0.5 grid h-5 w-5 place-items-center rounded border transition ' +
                (isOn
      ...
      {steps.map((s, i) => (
        <li key={i} className="grid gap-2">
          <div className="flex items-start gap-3">
            <div className="mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-orange-500 text-xs font-bold text-white shadow">
              {i + 1}
            </div>
  ...
  return (
    <div className="pl-9">
      <button
        ...
        className="h-8 rounded-full bg-orange-500 px-3 text-xs font-semibold text-white shadow hover:bg-orange-600"
      >
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div
            className="absolute inset-x-0 bottom-0 border-t border-amber-100 bg-white md:inset-x-6 md:inset-y-6 md:rounded-3xl md:border"
              ...
              <ol className="space-y-6">
                {steps.map((s, i) => (
                  <li key={i} className="space-y-3">
                    <div className="flex items-start gap-3">
                      <div className="mt-1 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-orange-500 text-sm font-bold text-white shadow">
                        {i + 1}
                      </div>
