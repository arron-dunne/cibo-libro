...
describe("hasPaywallFlag (unit)", () => {
  ...
  it("detects paywall at root level", () => {
    expect(hasPaywallFlag({ isAccessibleForFree: false })).toBe(true);
  });
  ...
  it("detects paywall inside arrays", () => {
    expect(
      hasPaywallFlag([{ foo: "bar" }, { isAccessibleForFree: false }]),
    ).toBe(true);
  });

  it("returns false when flag is true", () => {
    expect(hasPaywallFlag({ isAccessibleForFree: true })).toBe(false);
  });

  it("returns false when flag is missing", () => {
    expect(hasPaywallFlag({ "@type": "Recipe" })).toBe(false);
  });
});
